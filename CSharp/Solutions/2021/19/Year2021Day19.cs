...
namespace AdventOfCode.Solutions._2021._19;

[UsedImplicitly]
public class Year2021Day19 : ISolution
{
    public object Part1(IEnumerable<string> input)
    {
        var (count, _) = GetScanners(ParseInput(input));
    ...
    {
        var (_, scanners) = GetScanners(ParseInput(input));

        return GetDistances(scanners);
    }

    private static List<Vector3[]> ParseInput(IEnumerable<string> input)
    {
    ...
    private static (long, List<Vector3>) GetScanners(List<Vector3[]> scans)
    {
        ...
        var scanners = new List<Vector3> { new(0, 0, 0) };
        ...
        return (system.Length, scanners);
    }

    private static long GetDistances(List<Vector3> scanners)
    {
        var distances = from s in scanners
            from s2 in scanners
            select Math.Abs(s.X - s2.X) + Math.Abs(s.Y - s2.Y) + Math.Abs(s.Z - s2.Z);

        return (long)distances.Max();
    }
}