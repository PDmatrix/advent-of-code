...
(I'm an engineer, not a physicist!) Due to the aforementioned Planck lengths, the rope must be quite short; in fact, the head (```H```) and tail (```T```) must *always be touching* (diagonally adjacent and even overlapping both count as touching):
